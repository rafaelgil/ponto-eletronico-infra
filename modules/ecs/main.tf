...
======*/
resource "aws_cloudwatch_log_group" "ponto_eletronico_app" {
  name = "ponto_eletronico_app"

  tags = {
    Environment = var.environment
    Application = "ponto_eletronico_app"
  }
}
...
======*/
resource "aws_ecr_repository" "ponto_eletronico_app" {
...
data "template_file" "web_task" {
  ...
  vars = {
    image             = "${aws_ecr_repository.ponto_eletronico_app.repository_url}:latest"
    log_group         = aws_cloudwatch_log_group.ponto_eletronico_app.name
  }
}
...
resource "aws_alb" "alb_ponto_eletronico_app" {
  name            = "${var.environment}-alb-ponto-eletronico-app"
  ...
  tags = {
    Name        = "${var.environment}-alb-ponto_eletronico_app"
    Environment = var.environment
  }
}

resource "aws_alb_listener" "ponto_eletronico_app" {
  load_balancer_arn = aws_alb.alb_ponto_eletronico_app.arn
  port              = "80"
  protocol          = "HTTP"
...
resource "aws_cloudwatch_metric_alarm" "service_cpu_high" {
  alarm_name          = "${var.environment}_ponto_eletronico_app_web_cpu_utilization_high"
